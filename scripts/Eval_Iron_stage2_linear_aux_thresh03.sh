#!/bin/bash
# 评估消融：主实验 checkpoint (iron_stage2_linear)，仅将辅助头置信度阈值改为 0.3。
# 结果写入 ...-iron_stage2_linear_auxT03/，与主实验 0.5 分开对比。
...
ABLATION_DIR="./checkpoints/${SETTING}-iron_stage2_linear_auxT03"
...
python run_eval.py \
  ...
  --model_comment iron_stage2_linear_auxT03 \
  --llm_layers 32 \
  --itr 1 \
  --output_aux_semantic \
  --aux_confidence_threshold 0.3 \
  --zero_threshold -1
