#!/bin/bash
# 评估消融：主实验 checkpoint (iron_stage2_linear)，仅将辅助头置信度阈值改为 0.7。
...
ABLATION_DIR="./checkpoints/${SETTING}-iron_stage2_linear_auxT07"
...
python run_eval.py \
  ...
  --model_comment iron_stage2_linear_auxT07 \
  --llm_layers 32 \
  --itr 1 \
  --output_aux_semantic \
