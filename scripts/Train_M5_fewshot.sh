#!/bin/bash
# Few-shot 泛化：加载主模型（iron_stage2_linear），在 M5 上只用少量样本微调，再评估。
# 使用 M5 训练集的 10% 作为 few-shot 样本，训练 5 个 epoch，保存到 iron_stage2_linear_m5_fewshot。
...
comment='iron_stage2_linear_m5_fewshot'
...
echo "========== Few-shot：主模型 -> M5 微调 (${percent}% 训练集, ${train_epochs} epoch) =========="
...
echo ""
echo "Few-shot 训练完成。评估: bash scripts/Eval_M5_fewshot.sh"
