#!/bin/bash
# 基于 Iron.sh，仅改为软掩码：0 值日回归损失权重 0.1（非 0 日仍为 1），避免硬掩码导致 0 值日完全不更新回归头，保持梯度流动。
# 其余配置与 Iron.sh 完全一致（loss=JointMaskedAux，单层回归头，无 MAE/MLP）。
