...
def get_parser():
    ...
    p.add_argument('--print_prompt_once', action='store_true', help='评估时打印第一条喂入 LLM 的 Prompt（用于确认是否随窗口动态变化）')
...
def main():
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            ...
            need_aux_semantic = (
                args.model in ('TimeLLM', 'TimeLLM_TransformerOnly') and getattr(args, 'output_aux_semantic', False)
    ...
    try:
        ...
        pass
    lines.append('Eval device: {} | aux_confidence_threshold={} | aux_smooth_kernel={} | zero_threshold={}'.format(
        str(device),
    ...
    if aux_conf_thresh > 0 and len(all_aux_logits_list) > 0:
        ...
        if pred.ndim == 3 and logits.ndim == 3 and logits.shape[1] == pred.shape[1]:
            ...
            if logits.size == n_win * n_ch * pred_len_dim * 2:
                ...
                aux_4d = np.broadcast_to(logits[:, None, :, :], (n_win, n_ch, pred_len_dim, 2)).copy()
    ...
    lines.append('Test 整体  MAE_nonzero = {:.6f}  RMSE_nonzero = {:.6f}'.format(mae_nonzero_all, rmse_nonzero_all))
    lines.append('各指标:')
    for i in range(n_channels):
    ...
    if getattr(test_set, 'data_x', None) is not None and getattr(test_set, 'inverse_transform', None) and pred.shape[0] > 0:
        try:
            ...
            try:
                semantic_path = os.path.join(ckpt_dir, f'semantic_output{tag_suffix}.txt')
