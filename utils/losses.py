...
class JointMaskedMSEAuxBCE(nn.Module):
    ...
    """
    def __init__(self, lambda_weight=1.0, eps=1e-8, use_raw_mask=True, use_mae_for_num=False, mask_zero_weight=0.0, num_loss_scale=1.0):
        super().__init__()
    ...
    def forward(self, preds_num, preds_aux, targets_num, targets_aux, raw_targets_num=None):
        ...
        if targets_aux.dim() == 3:
            # 点对点：preds_aux 可能为 (B*N, pred_len, 2) 或 多元 channel_mixing 下 (B, pred_len, 2)
            B, pred_len, N = targets_aux.shape
            probs = preds_aux.softmax(dim=-1)[..., 1]  # (B*N, pred_len) 或 (B, pred_len)
            if probs.numel() == B * N * pred_len:
                probs = probs.view(B, N, pred_len).permute(0, 2, 1)  # (B, pred_len, N)
            elif probs.numel() == B * pred_len:
                # 多元 + channel_mixing：辅助头输出 (B, pred_len, 2)，同一预测复用到 N 通道
                probs = probs.unsqueeze(-1).expand(-1, -1, N)  # (B, pred_len, N)
            else:
                raise RuntimeError(
